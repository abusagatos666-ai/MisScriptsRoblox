-- [[ JD_PVP V18 - CONFIGURACIÓN EXTREMA (FIXED ALLIES) ]] --
...
RunService.Heartbeat:Connect(function()
    ...
    if char and char:FindFirstChild("Humanoid") and char:FindFirstChild("HumanoidRootPart") then
        ...
        -- SECCIÓN ACTUALIZADA: AIMBOT SOLO ENEMIGOS
            ...
            for _, p in pairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if p.Character.Humanoid.Health > 0 then
                        ...
                        -- FILTRO DE ALIADOS (Equipo y Tripulación)
                        local esAliado = false
                        if p.Team == LocalPlayer.Team then 
                            esAliado = true 
                        end
                        
                        -- Verificación adicional para Blox Fruits (Crew)
                        local myCrew = LocalPlayer:FindFirstChild("Leaderstats") and LocalPlayer.Leaderstats:FindFirstChild("Crew")
                        local pCrew = p:FindFirstChild("Leaderstats") and p.Leaderstats:FindFirstChild("Crew")
                        if myCrew and pCrew and myCrew.Value ~= "" and myCrew.Value == pCrew.Value then
                            esAliado = true
                        end

                        if not esAliado then
